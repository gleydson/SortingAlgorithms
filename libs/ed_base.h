...
class sfText : public sf::Text
{
  ...
  private:
    ...
    {
        const std::string _path = "libs/ed_font.ttf";
        static sf::Font font;
        ...
        if (init)
        {
            ...
            if (!font.loadFromFile(_path))
                std::cerr << "Font " << _path << " not found." << std::endl;
        }
        return &font;
    }
};
...
class MyWindow : public sf::RenderWindow
{
  ...
  private:
    ...
    MyWindow() : sf::RenderWindow(sf::VideoMode(largura, altura), "SortingAlgorithms")
    {
...
class MPlayer
{
  private:
    ...
    MPlayer() : _buffer(100, sf::Texture())
    {
    }
  ...
  private:
    ...
    void print_label()
    {
        ...
        std::string title_left = " Jump|First|Current|Last|";
